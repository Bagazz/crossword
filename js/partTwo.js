...
function SelectThisWord(event)
{
	...
	document.getElementById("wordinfo").innerHTML = ((CurrentWord <= LastHorizontalWord) ? "Вправо, " : "Вниз, ") + WordLength[CurrentWord] + " букв.";
...
function OKClick()
{
	...
	{
		document.getElementById("worderror").innerHTML = "Введенное вами слово содержит недопустимые символы. Пожалуйста, вводите только буквы в поле выше.";
		document.getElementById("worderror").style.display = "block";
		return;
	}
	if (TheirWord.length < WordLength[CurrentWord])
	{
		document.getElementById("worderror").innerHTML  = "Вы набрали недостаточно букв. В этом слове " + WordLength[CurrentWord] + " букв.";
		document.getElementById("worderror").style.display = "block";
		return;
	}
	if (TheirWord.length > WordLength[CurrentWord])
	{
		document.getElementById("worderror").innerHTML = "Вы набрали слишком много букв. Это слово имеет " + WordLength[CurrentWord] + " букв.";
		document.getElementById("worderror").style.display = "block";
		return;
	}
...
function CheckClick()
{
	...
	if (ErrorsFound > 0 && EmptyFound > 0)
		document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибок" : " ошибка") + " и " + EmptyFound + (EmptyFound > 1 ? " неправильных слов " : " неправильное слово") + " найдено.";
	else if (ErrorsFound > 0)
		document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибок" : " ошибка") + " найдено.";
	else if (EmptyFound > 0)
		document.getElementById("welcomemessage").innerHTML = "Ошибок нет, но " + EmptyFound + (EmptyFound > 1 ? " слов нужно" : " слово нужно") + " найти.";
